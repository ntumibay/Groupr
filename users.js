...
export const addEvents = async (userId, event) => {
  ...
  const validKeys = ['title', 'startDate', 'endDate', 'startTime', 'endTime', 'description'];
  for (let i = 0; i < eventKeys.length; i++){
    if (!validKeys.includes(eventKeys[i])){
      throw "Error: event can only contain title, startDate, endDate, startTime, endTime and description";
    }
  }
